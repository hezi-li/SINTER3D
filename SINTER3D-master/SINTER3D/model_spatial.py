...
class Model():
    def __init__(self, adata_st_list_raw, adata_st, adata_basis, slice_idx,
        ...
        # 2. Read config values and override defaults
        if config is not None:
            config_params = load_config(config)
            print("Config parameters loaded:", config_params)
            params.update(config_params)  # Config values take priority over defaults

        # 3. Assign all values to self
        for k, v in params.items():
            setattr(self, k, v)

        # 4. Print the final effective parameters for debugging
        print("\nFinal parameters used in Model:")
        for k in params.keys():
            print(f"  {k}: {getattr(self, k)}")
        print()

        # ---- Fixed initialization starts here ----
        set_seed(self.seed)
        ...
        # Validate the hidden_dims format
    ...
    def train(self, report_loss=True, step_interval=1000, min_delta=1e-4):
        ...
        best_state = None  # Best model parameters
        wait_count = 0  # Wait counter for patience accumulation
        ...
        for step in tqdm(range(self.net.training_steps)):
            ...
            # Stop early if the wait count exceeds patience
    ...
    def eval(self, adata_st_list_raw, save=False, output_path="./results"):
        ...
        n_spots = 0
        adata_st_decon_list = []
        decon_results_all = []  # Used to save all deconvolution results
        ...
        for i, adata_st_i in enumerate(adata_st_list_raw):
            ...
            # Generate deconvolution results
            ...
            # Append deconvolution results to the list
            decon_results_all.append(decon_res)

            n_spots += adata_st_i.shape[0]
            adata_st_decon_list.append(adata_st_i)

        # If requested, merge and save all deconvolution results
        if save:
            # Merge deconvolution results into a DataFrame
    ...
    def inference_latent(
        ...
        with torch.no_grad():
            ...
            if slice_key not in adata_new.obs.columns:
                # If slice_key is unavailable, fall back to the original single-slice logic
            ...
            for k, new_slice in enumerate(unique_new_slices):
                ...
                # Note: only use embeddings corresponding to valid_slice_ids
                ...
                # New embedding IDs start after the original embedding table
            ...
            # Save the mapping for easier inspection
