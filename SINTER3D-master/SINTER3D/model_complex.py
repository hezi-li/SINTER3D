...
def load_config(config_name_or_path):
    """Simple configuration loader that automatically handles multi-configuration files."""
...
class Model():
    def __init__(self, adata_st_list_raw, adata_st, adata_basis, slice_idx,
        ...
        # 2. Load config values and override defaults
        if config is not None:
            config_params = load_config(config)
            print("Config parameters loaded:", config_params)
            params.update(config_params) 

        # 3. Assign values to class attributes
        for k, v in params.items():
            setattr(self, k, v)

        # 4. Print the final effective parameters at startup
        ...
        # Use self.hidden_dims here; it may have been overridden by the config
    ...
    def train(self, report_loss=True, step_interval=1000, min_delta=1e-4):
        ...
        best_loss = float('inf')     # Current best loss
        best_state = None            # Save the best model state
        wait_count = 0               # Wait counter
        ...
        for step in tqdm(range(self.net.training_steps)):
            ...
            if best_loss - loss.item() > min_delta:
                # Loss improved significantly; update the best model state
                best_loss = loss.item()
                best_state = {
            ...
            # Check whether patience has been exceeded
    ...
    def eval(self, adata_st_list_raw, save=False, output_path="./results"):
        ...
        # Process and save latent embeddings (representation.csv)
        ...
        n_spots = 0
        adata_st_decon_list = []
        decon_results_all = []  # Added: collect deconvolution results from all slices

        for i, adata_st_i in enumerate(adata_st_list_raw):
            adata_st_i = adata_st_i.copy()

            # Index handling; keep the original logic
            ...
            # Generate deconvolution results for the current slice
            decon_res = pd.DataFrame(
                b[n_spots:(n_spots + adata_st_i.shape[0]), :],
                columns=self.celltypes
            )
            decon_res.index = adata_st_i.obs.index
            decon_results_all.append(decon_res)  # Added: collect results for the current slice

            # Merge into adata; keep the original logic
        ...
        if save:
            ...
            # Merge and save deconvolution results from all slices
    ...
    def inference_latent(
        ...
        with torch.no_grad():
            ...
            # self.adata_st.obs["slice"] has already been remapped to 0, 1, ..., n_train - 1 in __init__
