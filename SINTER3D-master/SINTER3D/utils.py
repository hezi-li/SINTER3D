...
from SINTER3D.align_tools import *
...
def preprocess(adata_st_list_input, # list of spatial transcriptomics (ST) anndata objects
    ...
    for i in range(len(adata_st_list)):
        ...
        adata_st_new.obs.index = adata_st_new.obs.index
...
def align_spots(
    ...
    # CAST default parameters
    cast_output_path="./CAST_alignment",
    cast_gpu=None,
    cast_diff_step=5,
    cast_epochs=400,
    # CAST affine parameters
    cast_iterations=500,
    ...
    if plot:
        ...
        if plot_colors is not None:
            if len(plot_colors) < len(adata_st_list):
                raise ValueError(
                    f"plot_colors has {len(plot_colors)} colors, "
                    f"but {len(adata_st_list)} slices are provided."
                )
            colors_list = plot_colors[:len(adata_st_list)]
        else:
            cmap = cm.get_cmap(plot_cmap, len(adata_st_list))
            colors_list = [
    ...
    if (method == "icp") or (method == "ICP"):
        ...
        for adata in adata_st_list:
            ...
            if data_type == "Visium":
                ...
                loc = np.asarray(adata.obsm[coor_key])
                tree = cKDTree(loc)
                dists, _ = tree.query(loc, k=2)
                median_nn = float(np.median(dists[:, 1]))
                radius = median_nn * 3
                
                # KDTree neighbor counting without allocating an N x N matrix
                n_neighbors = tree.query_ball_point(loc, r=radius, return_length=True) - 1
                
                median_n_neighbors = np.median(n_neighbors)
                edge_threshold = max(8, median_n_neighbors * 0.4)
                edge = ((n_neighbors > 1) & (n_neighbors < edge_threshold)).astype(np.float32)
                
                print(f"  Detected {int(edge.sum())} edge cells out of {len(edge)} "
                    f"(median neighbors: {median_n_neighbors:.0f}, threshold: {edge_threshold:.0f})")
            else:
                loc_x = adata.obs.loc[:, ["array_row"]]
    ...
    elif method.lower() == "cast":
        adata_st_list = align_slices_cast(
            adata_st_list,
            coor_key=coor_key,
            plot=plot,
            cast_output_path=cast_output_path,
            cast_gpu=cast_gpu,
            cast_diff_step=cast_diff_step,
            cast_epochs=cast_epochs,
            cast_iterations=cast_iterations,
            cast_dist_penalty1=cast_dist_penalty1,
            cast_bleeding=cast_bleeding,
            cast_d_list=cast_d_list,
            cast_attention_params=cast_attention_params,
            cast_dist_penalty2=cast_dist_penalty2,
            cast_alpha_basis_bs=cast_alpha_basis_bs,
            cast_meshsize=cast_meshsize,
            cast_iterations_bs=cast_iterations_bs,
            cast_attention_params_bs=cast_attention_params_bs,
            cast_mesh_weight=cast_mesh_weight,
            cast_alpha_basis=cast_alpha_basis,
        )
